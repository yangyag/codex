...
- 테스트 의무: 기능 구현 시 반드시 해당 기능을 커버하는 테스트 코드를 함께 추가하고 실행 결과를 확인한다.
- 테스트 범위: 자동화 테스트는 백엔드(서비스 레이어/웹 레이어) 중심으로 작성한다.
- 명명 규칙: 테스트 메서드명은 영어로 작성하고, 설명/주석/어노테이션 메시지에는 한글을 사용할 수 있다.
- 설계 원칙: 외부 연동/도메인 규칙 등 변경 가능성이 높은 경계에는 인터페이스(포트) + 구현체(어댑터)로 분리한다. 복잡한 도메인 객체 생성에는 팩토리/빌더를 사용하고, 주요 유스케이스는 커맨드/핸들러로 캡슐화해 재사용/검증을 용이하게 한다.
